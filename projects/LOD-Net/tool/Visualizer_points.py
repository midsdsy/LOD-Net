...
__all__ = ["ColorMode", "VisImage", "Visualizer_bordermask"]
...
class Visualizer_bordermask:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        chosen_coordinate = predictions.chosenP if predictions.has("chosenP") else None
    ...
    def overlay_instances(
        ...
        *,
        ...
        if chosen_coordinate is not None:
            if num_instances:
                assert len(chosen_coordinate) == num_instances
            else:
                num_instances = len(chosen_coordinate)
            chosen_coordinate = self._convert_chosen_coordinate(boxes, chosen_coordinate)
        ...
        if chosen_coordinate is not None:
            for chosen_coordinate_per_instance in chosen_coordinate:
                self.draw_keypoints(chosen_coordinate_per_instance)
    ...
    def draw_keypoints(self, keypoints):
        ...
        for idx, keypoint in enumerate(keypoints):
            # draw keypoint
            x, y = keypoint
            self.draw_circle((x, y), color=_RED,radius=1.5)
            # import pdb
            # pdb.set_trace()
    ...
    def _convert_chosen_coordinate(self, boxes, chosen_coordinates):
        num_instances,_ = boxes.shape
        _, size,_ = chosen_coordinates.shape
        keypoints = []
        for idx in range(num_instances):
            box = boxes[idx]
            chosen_coordinate = chosen_coordinates[idx]
            x0, y0, x1, y1 = box
            width = x1 - x0
            height = y1 - y0
            keypoint_per = []
            x_range = torch.linspace(x0, x1, size)
            y_range = torch.linspace(y0, y1, size)
            y, x = torch.meshgrid(y_range, x_range)


            for x,y,p in zip(x.reshape(-1),y.reshape(-1),chosen_coordinate.reshape(-1)):
                if p:
                    keypoint_per.append([x,y])

            keypoints.append(keypoint_per)
            # import pdb
            # pdb.set_trace()
