...
def main(args):
    ...
    args.config = args.config if args.config is not None else "config/LOD_test.yaml"
    cfg.merge_from_file(args.config)
    base_predPath = args.pred_path if args.pred_path is not None else "./outputs/seman/other_method_results/SFA/"
    base_gtPath = args.gt_path if args.gt_path is not None else "./datasets/polyp/TestDataset"
    SCORE_THRESH_TEST = 0.5

    # datasets = ["CVC_ColonDB_val"]
    for dataset in datasets:
        ...
        for idx, inputs in tqdm(enumerate(data_loader)):
            ...
            pred_mask[pred_mask > 255 * SCORE_THRESH_TEST] = 255.
