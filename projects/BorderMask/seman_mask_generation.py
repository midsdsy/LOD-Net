...
def main(args):
    ...
    args.config = args.config if args.config is not None else "config/for_sota/bordermask_LOG_R_101_FPN_aug_class1_mstrain.yaml"
    args.weights = args.weights if args.weights is not None else "./polyp/final/bordermask_LOG_R_101_FPN_aug_class1_mstrain/model_final.pth"
    args.output_path = args.output_path if args.output_path is not None else "./polyp_eval/seman/final_mstest_gai/"
    # bordermask_LOG_R_101_FPN_aug_class1_mstrain_bf
    ...
    for dataset in datasets:
        ...
        for idx, inputs in tqdm(enumerate(data_loader)):
            ...
            seman_mask = seman_mask.sum(dim=0) / ((seman_mask > 0).sum(dim=0) + 1e-8)
            seman_mask = seman_mask.sigmoid().data.cpu().numpy()
            seman_mask = (seman_mask - seman_mask.min()) / (seman_mask.max() - seman_mask.min() + 1e-8)

            matplotlib.image.imsave(os.path.join(visPath, base_name), seman_mask, cmap='gray')
